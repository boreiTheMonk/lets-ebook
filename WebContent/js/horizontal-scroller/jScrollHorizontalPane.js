	...
	return this.each(
		function()
		{
			...
			if (jQuery(this).parent().is('.jScrollPaneContainer')) {
				...
				jQuery('>.jScrollPaneTrack222, >.jScrollArrowLeft, >.jScrollArrowRight', $c).remove();
				$this.css({'left':0});
				_jscr_originalPos[currentId] = -1;
			} else {
			...
			if (percentInView < 0.99) {
				...
				$container.append(
					jQuery('<div></div>').attr({'className':'jScrollPaneTrack222'}).css({'height':settings.scrollbarHeight+'px'}).append(
				...
				var $track = jQuery('>.jScrollPaneTrack222', $container);
				...
				var $drag = jQuery('>.jScrollPaneTrack222 .jScrollPaneDrag', $container);
				...
				var evaluateIntervals = function(position, destX) { 
					...
					if((intervalMove == false) && (mouseWheelMove != true)) { 
							...
							fullTrackWidth = $('.jScrollPaneTrack222').width();
						...
						function (event, delta) {
							...
							if(trackIntervals.length > 1) {
								...
								if(delta < 0) {
									...
									if((parseInt($drag.width())+ parseInt(trackIntervals[_jscr_trackInt[currentId]])) > parseInt($('.jScrollPaneTrack222').width())) {
										movePos = parseInt($('.jScrollPaneTrack222').width()) - $drag.width();
									}
